...
namespace DK\MerchantSuite {
	...
	use Httpful\Request as Httpful_Request;
	...
	class RequestSender
	{
		...
		public static function send($url, $authString, $payload, $method) {
			...
			switch ($method) {
				case "POST":
					$request = Httpful_Request::post($url);
					break;
				case "GET":
					$request = Httpful_Request::get($url);
					break;
				case "PUT":
					$request = Httpful_Request::put($url);
					break;
				case "DELETE":
					$request = Httpful_Request::delete($url);
					break;
			}
	...
	class WebHookConsumer
	{
		...
		public static function consumeToken($hook) {
			...
			$payload->APIResponse->ResponseCode = "0";
			$payload->APIResponse->ResponseText = "";
			$payload->TokenResp = json_decode($hook);
